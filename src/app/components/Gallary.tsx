...
export default async function Gallary({ topic = "curated", page }: Props) {
  ...
  if (!images || !images.total_results)
    return (
      <div className="h-16 text-center font-bold text-3xl">
        No images found!
      </div>
    );
  ...
  if (!page) page = "2";
    ...
    page = Math.min(
      photosWithBlurr.total_results / 15 + 1,
      Number(page) + 1
    ).toString();
  }
      ...
      <div className="mx-auto flex p-4 w-full justify-center">
        {photosWithBlurr.prev_page ? (
          <div className="font-semibold text-lg mx-2">
            <Link href={`/results/${topic}/${Number(page) - 1}`}>
              {" "}
              &lt;&lt;&lt;prev{" "}
            </Link>
          </div>
