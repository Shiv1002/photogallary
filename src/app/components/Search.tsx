...
export default function Search() {
  ...
  useEffect(() => {
    ...
    const his = searchHistorySchema.parse(
      JSON.parse(localStorage.getItem("photo-gallary") || "")
    );
  ...
  const pushToHistory = (new_search: string) => {
    ...
    if (filtered?.length) {
      ...
      const newHis = history?.map((s) => {
        if (s.term === new_search) s.frequency += 1;
        return s;
      });
      setHistory(newHis);
    } else {
      console.log("adding new term");
      if (history) setHistory([...history, { term: new_search, frequency: 1 }]);
    }
  };
